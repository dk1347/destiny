...
| 일주 경계 | 00:00 | MVP의 기본값. 다른 전통은 별도 프로필로 제공한다. |
| 23:00–00:59 | 민간 날짜와 동일한 일주 | 야자시 일주 변경은 기본값에 적용하지 않는다. |
| 시지 | 2시간 단위 | 자시 23:00–00:59, 축시 01:00–02:59 등의 표준 구간. |

`true_solar_time`, `zi_hour_23`, 대운 세부 규칙은 검증이 끝난 뒤 별도 프로필로 추가한다. 기본 결과와 다른 프로필 결과를 섞어 보여주지 않는다.
...
5. 이후 `true_solar_time` 및 야자시 대안 프로필을 추가한다.
