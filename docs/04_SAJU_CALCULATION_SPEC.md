...
`true_solar_time`과 대운 세부 규칙은 검증이 끝난 뒤 별도 프로필로 추가한다. 기본 결과와 다른 프로필 결과를 섞어 보여주지 않는다.

현재 코어가 제공하는 프로필 식별자는 다음과 같다.

| 프로필 | 일주 경계 | 상태 |
| --- | --- | --- |
| `kr_standard_v1` | 23:00 자시 시작 | 기본값 |
| `midnight_v1` | 00:00 자정 | 비교용 대안 |

두 프로필은 현재 법정 현지시만 지원한다. 진태양시 보정은 별도
time-resolution 근거와 프로필이 준비되기 전에는 제공하지 않는다.
